...
import type { DotPattern, Model3DParams, ExportParams, QualityReport } from '@/types'
...
interface ModelViewerProps {
  ...
  onQualityAssessment?: (report: QualityReport) => void
}
...
export const ModelViewer: React.FC<ModelViewerProps> = ({
  ...
  useEffect(() => {
    ...
    try {
      ...
      setIsLoading(false)
      // Trigger quality assessment after model loads if enabled
      if (showQualityInspection) {
        setTimeout(() => performQualityAssessment(), 100)
      }
    }
  }, [pattern, model3DParams, onError, showQualityInspection, performQualityAssessment])
  ...
  const performQualityAssessment = useCallback(async () => {
    if (!meshRef.current || !showQualityInspection) return

    setIsAnalyzing(true)
    
    try {
      const modelId = `model-${Date.now()}`
      const report = QualityAssessmentService.assessQuality(meshRef.current, modelId)
      
      setQualityReport(report)
      if (onQualityAssessment) {
        onQualityAssessment(report)
      }
      
    } catch (error) {
      if (onError) {
        onError(`Quality assessment failed: ${error instanceof Error ? error.message : 'Unknown error'}`)
      }
    } finally {
      setIsAnalyzing(false)
    }
  }, [showQualityInspection, onQualityAssessment, onError])
      ...
      <div className="viewer-controls">
        ...
        {showQualityInspection && (
          <div className="control-group">
            ...
            <div className="control-buttons">
              <button 
                onClick={performQualityAssessment}
            ...
            {qualityReport && (
              <div className="quality-results">
                <div className="quality-score">
                  <span className={`score ${qualityReport.overallScore >= 80 ? 'good' : qualityReport.overallScore >= 60 ? 'warning' : 'poor'}`}>
                ...
                {qualityReport.recommendations.length > 0 && (
                  <div className="quality-recommendations">
                    ...
                    <ul>
                      {qualityReport.recommendations.slice(0, 2).map((rec, index) => (
                ...
                {qualityReport.warnings.length > 0 && (
                  <div className="quality-warnings">
                    <h5>Warnings:</h5>
                    <ul>
                      {qualityReport.warnings.slice(0, 2).map((warning, index) => (
